...
fun angleInRadian(grad: Int, min: Int, sec: Int): Double {
    ...
    return (grad + min / 60.0 + sec / (60.0 * 60.0)) * PI / 180.0
}
...
fun thirdDigit(number: Int): Int {
    val d1 = number % 10
    val d2 = (number - d1) % 100
    val d3 = (number - d2 - d1) % 1000
    return d3 / 100
}
...
fun accountInThreeYears(initial: Int, percent: Int): Double {
    val y1 = initial + initial * (percent / 100.0)
    val y2 = y1 + y1 * (percent / 100.0)
    val y3 = y2 + y2 * (percent / 100.0)
    return y3
}
...
fun numberRevert(number: Int): Int {
    val d3 = number % 10
    val d2 = (number - d3) % 100
    val d1 = number - d2 - d3
    return d3 * 100 + d2 + d1 / 100
}